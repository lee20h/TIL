...
두 가지가 시간이 가장 오래걸렸고 이해하기도 어려웠다. 먼저 박스 채우기를 살펴보자.
```
...
## JQuery [참고](http://tcpschool.com/jquery/intro)
...
### 요소 탐색
**트리 탐색**  
특정 요소들과의 관계를 통해 선택하길 원하는 요소까지 DOM 트리를 검색해 나아가는 과정
...
DOM 트리에서 특정 요소의 부모 요소를 포함한 상위의 요소를 탐색하기 위한 메소드 종류  

- `.parent()` : 선택한 요소의 부모 요소를 선택한다. 선택자를 인수로 전달하여, 전달받은 선택자에 해당하는 부모 요소만 선택 가능하다.
```
$(function() {
    $("button").on("click", function() {
...
}); //.css() 메소드는 선택한 요소에 인수로 전달받은 스타일 설정
```
- `.parents()` : 선택한 요소의 조상 요소를 모두 선택한다. 선택자를 인수로 전달하여, 전달받은 선택자에 해당하는 조상 요소만을 선택할 수도 있다.
```
$(function() {
    $("button").on("click", function() {
...
```
- `.parentUntil()` : 선택한 요소의 조상 요소 중 전달받은 선택자에 해당하는 요소 바로 이전까지의 요소만 모두 선택한다. 선택자를 인수로 전달하지 않으면, `.parents()`와 같이 선택한 요소의 조상 요소를 모두 선택한다.
```
$(function() {
    $("button").on("click", function() {
...
```
- `.closest()` : 자신을 포함한 조상 요소 중에서 전달받은 선택자에 해당하는 요소의 집합에서 가장 첫 번째 요소를 선택한다. 요소의 집합을 구하는 방식은 `.parents()`와 비슷하지만, 해당 요소의 조상 요소뿐만 아니라 해당 요소 자신까지도 검사한다는 점이 다르다.
```
$(function() {
...
- .siblings() : 선택한 요소의 형제 요소 중 지정한 선택자에 해당하는 요소를 모두 선택한다.
```
$(function() {
    $("button").on("click", function() {
...
- .nextAll() : 선택한 요소의 다음에 위치한 형제 요소를 모두 선택한다. 선택자를 인수로 전달하여, 전달받은 선택자에 해당하는 형제 요소만을 선택할 수도 있다.
...
- .nextUntil() : 선택한 요소의 형제 요소 중에서 전달받은 선택자에 해당하는 요소 바로 이전까지의 요소만을 모두 선택한다. 선택자를 인수로 전달하지 않으면, `.nextAll()`와 같이 선택한 요소의 다음에 위치한 형제 요소를 모두 선택한다.
...
- .prev() : `.next()`와 정반대로 동작
- .prevAll() : `.nextAll()`과 반대로 동작 
- .prevUntil() : `.nextUntil()`과 반대로 동작

자손 요소의 탐색  

- .children() : 선택한 요소의 자식 요소를 모두 선택한다. 선택자를 인수로 전달하며, 전달받은 선택자에 해당하는 자식 요소만을 선택 가능하다.
```
$(function() {
    $("button").on("click", function() {
...
- .find() : 선택한 요소의 자손 요소 중에서 전달받은 선택자에 해당하는 자손 요소를 모두 선택한다. 별표(`*`)을 인수로 전달하여, 선택한 요소의 자손 요소를 모두 선택 가능하다.
```
$(function() {
    $("button").on("click", function() {
...
기타 탐색 메소드
- .add() : 선택한 요소의 집합에 전달받은 요소 추가
- .each() : 선택한 요소 집합의 요소마다 전달받은 콜백 함수 실행
- .end() : 마지막으로 실행한 메소드의 실행 전 상태로 선택한 요소의 집합을 복원해준다.
- .offsetParent() : 선택한 요소를 위치시킬 때 기준으로 사용된 조상 요소를 선택할 수 있다.
- .contents() : 선택한 요소의 자식 요소를 텍스트 노드와 주석 노드까지 모두 포함하여 선택한다.

필터링 메소드  

- .first() : 선택한 요소 중 첫 번째 요소 선택
- .last() : 선택한 요소 중 마지막 요소 선택
- .eq() : 선택한 요소 중 전달받은 인덱스에 해당하는 요소 선택. 선택한 요소 집합의 맨 처음 요소를 인덱스 0으로 놓고 검색
- .filter() : 선택한 요소중에서 전달받은 선택자에 해당하거나, 함수 호출의 결과가 참인 요소를 모두 선택
- .not() : 선택한 요소중에서 전달받은 선택자에 해당하거나, 함수 호출의 결과가 참인 요소를 제외한 나머지 모두 선택
- .has() : 선택한 요소 중 전달받은 선택자에 해당하는 요소를 자손 요소로 가지고 있는 요소 모두 선택
- .is() : 선택한 요소 중 전달받은 선택자에 해당하는 요소가 하나라도 존재하면 참 반환
- .map() : 선택한 요소 집합의 각 요소마다 지정된 콜백 함수 실행, 그 반환값으로 구성된 JQuery 객체 반환
- .slice() : 선택한 요소 중 전달받은 인덱스 범위에 해당하는 요소만 선택

### 이벤트 처리
이벤트란?

웹 페이지에서 특정 동작이 발생하여, 웹 브라우저가 그 사실을 알려주는 것을 의미  
...
이벤트 핸들러를 특정 요소에 연결하는 것으로 아래와 같다.
```
...
제이쿼리 1.7 이후 부터는 `.bind()`나 `.click()`을 사용해도 내부적으로 `.on()`을 이용해서 연결한다.
```
$(function() {
    $("body").on({          // <body>요소에
...
특정 요소와 이벤트를 연결하기 위해 사용한다. 그의 특징으로는
...
.on()와 같은 문법으로 사용하되, 처음 한번 이후에는 실행되지 않는다.  

`.off()` 메소드  

더는 사용하지 않는 이벤트와의 연결을 제거한다.  

이벤트의 위임  

다수의 요소에 공통으로 적용되는 이벤트 핸들러를 공통된 조상 요소에 단 한 번만 연결하면 동작할 수 있도록 한다.
```
$(function() {
...
```
`<ul>`요소의 자식 요소 중 `<a>`요소에 대해 요소마다 각각 이벤트 핸들러를 직접 연결하는 소스이다.  

이후 요소마다 직접 등록하는 이벤트 핸들러는 현재 존재하는 `<a>`요소에만 연결되며 새롭게 추가되는 요소에는 연결되지 않는 모습을 보여준다.
```
$(function() {
...
따라서 이 소스를 아래와 같이 이벤트의 위임하는 형식으로 바꾸게 되면 이렇게 된다.
...
`.on()`는 해당 요소에 첫 번째 인수로 전달받은 이벤트가 전파되었을 때, 그 이벤트를 발생한 요소가 두 번째 인수로 전달받은 선택자와 같은지 검사한다. 같으면 연결된 이벤트 핸들러를 실행한다.  

이런 식으로 이벤트의 위임을 이용하면 앞으로 추가 될 자식 요소도 자동으로 연결된다.  

이벤트의 전파  

이벤트가 발생했을 때, 브라우저가 이벤트 핸들러를 실행시킬 대상 요소를 결정하는 과정이다. 이벤트가 DOM 객체나 HTML 문서의 요소에서 일어나게되면 대상 객체를 결정하기 위해서 이벤트의 전파가 일어나는데, 이것을 이벤트 버블링이라고 한다.  

이벤트 버블링은 발생 요소부터 DOM 트리를 따라 위쪽으로 올라가며 전파되는 과정이다. 즉, 이벤트가 발생한 요소에 연결된 이벤트 핸들러가 실행된 후, 그 부모 요소에 연결된 핸들러가 실행된다. 그런 식으로 Document 객체까지 계속 전파가 된다. 이 전파를 통해서 이벤트의 위임이 가능하다.  
...
| .dbclick() | 자바스크립트의 "dblclick" 이벤트와 이벤트 핸들러를 연결하거나, 해당 요소에 "dblclick" 이벤트를 발생시킴. |
...
[이펙트 효과](http://tcpschool.com/jquery/jq_effect_showingHiding)는 해당 사이트에서 보고 공부해보자. (fade, slide 등)  

### Ajax  

Ajax는 따로 한번 정리를 할 생각이므로, JQuery에서 사용하는 예제와 메소드만 정리하도록 하자.  
...
Ajax에서 서버와의 비동기식 통신을 위해 form 요소를 통해 입력받은 데이터를 직렬화(Serialization)하여 전송한다. 이때 직렬화란 입력받은 여러 데이터를 하나의 쿼리 문자열로 만드는 것을 의미한다. 따라서 form 요소를 통해 입력받은 데이터를 한 번에 서버로 보낼 수 있다.

직렬화는 `.serialize()`와 `.serializeArray()`을 통해 이루어진다. `.serialize()`메소드는 HTML form 요소를 통해 입력된 데이터를 쿼리 문자열로 변환하지만 `.serializeArray()`는 문자열이 아닌 배열 객체로 반환한다.  
...
![img_ajax_other_application](../../docs/.vuepress/public/images/Ajax/img_ajax_other_application.jpg)   

![img_ajax_ajax_application](../../docs/.vuepress/public/images/Ajax/img_ajax_ajax_application.jpg)  
