...
| 날짜 | 공부 내용 |
|------|----------|
| 1일 | Code Review: Experience, Gerrit, Auto Scaling |
| 2일 | PS |
| 3일 | PS |
| 4일 | Mocha.js |
| 5일 | PS, 소프트웨어공학 프로젝트 |
| 6일 | PS |
| 7일 | Travis CI & docker를 이용한 AWS Elastic beanstalk 배포 |
| 8일 | AWS 배포환경 구축 |
| 9일 | Thread |
| 10일 | 소프트웨어공학 시험 |
| 11일 | 컴퓨터네트워크 시험 |
| 12일 | PS |
| 13일 | AWS Elastic Beanstalk 배포 환경 구성 |
| 14일 | AWS DevOps + ECR 배포 환경 구축 |
| 15일 | OSS 프로젝트 |
| 16일 | PS |
| 17일 | PS | 
| 18일 | Node js |
...
| 22일 | PS |
| 23일 | Babel |
| 24일 | Express Middleware |
| 25일 | MVC 패턴 |
| 26일 | MVC 패턴 |
| 27일 | PS |
| 28일 | MVC 패턴 |
| 29일 | Pug |
| 30일 | MongoDB 포터블 설치, 연결 |
| 31일 | Mongoose Model |
...
소프트웨어공학에서 소프트웨어 설계 단계인 `계획 -> 요구사항 분석 -> 소프트웨어 디자인 -> 구현 -> 테스팅 -> 운용과 관리` 중 요구사항 분석과 소프트웨어 디자인에 대해서 시험을 보았다. 그에 따라서 요구사항 분석에 쓰이는 모델링들과 요구사항 명세서, 소프트웨어 디자인에 필요한 설계 기법과 디자인 패턴 등을 주로 공부하였다.
...
Litmus Online Judge는 전북대학교 컴퓨터공학부 선배들이 만든 프로젝트로, 온라인 저지 사이트로써 문제를 내고 코딩해서 풀 수 있는 식으로 이루어져 있다. 개발된지 상당히 되어 유지보수가 안되고 사용되어오다 보니 쌓인 데이터는 상당하나, 유지보수를 위한 문서나 내용을 이해하기엔 시간이 필요하다. 또한 개발자들은 모두 졸업하여 내용을 알려면 직접 코드를 읽고 따라가봐야한다.
...
기지국이 위치할 때 w로 기지국의 범위가 주어졌을 때 모든 공간에 기지국의 범위가 닿도록 할 때 최소 기지국의 갯수를 구하는 문제이다.

설치되지 않은 구간은 처음 / 첫 기지국 ~ 마지막 기지국 / 끝으로 나눠서 구했다.

- 처음 `1 ~ (첫 기지국 - w - 1)`
- 중간 `(직전 기지국 + w + 1) ~ (현재 기지국 - w - 1)`
- 끝 `(마지막 기지국 + w + 1) ~ N`
  
필요한 기지국은 구간의 길이를 전파의 도달 거리 * 2 + 1로 나뉘었을 때 몫을 구하도록 한다. 이 때 몫이 소숫점이 나오면 올림해주면 된다.

이렇게 해결하였고, 다른 사람의 풀이를 봐보자.
...
프로그래머스에서 주홍철님의 풀이로, a에 기지국의 전파가 닿지 않는 길이를 구한 뒤 기지국이 몇개 들어갈지 계산 한 뒤 값을 더해준다. 이러한 방식으로 쭉 진행하여 마지막 부분까지 추가적으로 더해주는 방식으로 문제를 해결하였다. 이 때 a가 음수가 나올 수 있으므로 max로 추가적인 연산을 취해줬다.

훨씬 짧고 간결하게 코딩이 되어서 여러 생각이 들게 하는 풀이였다.
...
Model, View, Controller를 분리하는 패턴인 MVC패턴은 node.js에서 express 프레임워크를 사용할 때 주로 사용된느 패턴이다.

Model은 Database의 모델들을 뜻하므로 주로 MongoDB를 사용하여 정의하고, 각 모델 하나하나마다 스키마를 정의한다.

View는 데이터들이 보이는 것들이다. 내가 사용한 것은 Express의 템플릿 엔진 중 Pug이다. Pug로 Html을 표시할 수 있다. Express의 route를 이용하여 URL과 pug를 연결하여 보여주도록 하였다.

Controller는 주로 함수를 뜻하며, 데이터들을 이용해서 행해지는 일련의 행위를 모아놓는다. 이 때, 모델마다 컨트롤러가 하나씩 배정된다. 모델마다 다른 함수를 사용하여 기능을 분리하는 것이다.
