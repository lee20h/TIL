...
| 1일 | SQLD 문제 풀이 |
| 2일 | SQLD 정리 |
| 3일 | KMP 문제 풀이 Censoring |
| 4일 | SQLD 시험 |
| 5일 | 네이버 D2 대학생 밋업: 백엔드 개발 정독 |
| 6일 | DP 문제 풀이 (카드 구매하기) |
| 7일 | 컴퓨터 네트워크 : road map, Network Layer, 소프트웨어공학 : 1~3 장 |
| 8일 | DP와 문자열 문제 해결 (퇴사, 버그잡이 꿍) |
| 9일 | OpenSource Software Development : 1장, Docker Compose |
| 10일 | 프로그래머스 코딩챌린지 9월 |
| 11일 | Travis CI |
| 12일 | 카카오 공채 코딩테스트 |
| 13일 | 해시 문제풀이(전화번호 목록) |
| 14일 | Travis AWS 연동, 유닉스 디렉토리와 파일, OSS 2장 |
| 15일 | 소프트웨어공학 : 4장 |
| 16일 | 컴퓨터네트워크 : Network Layer, 분산컴퓨팅 : 클라우드 컴퓨팅, 클라우드 서비스 모델과 배포 모델 |
| 17일 | 프로그래머스 PS | 
| 18일 | 프로그래머스 PS |
| 19일 | 프로그래머스 PS |
| 20일 | 코드포스 PS |
| 21일 | 컴퓨터네트워크 : Network Layer Protocol, 소프트웨어공학 : 애자일, XP, 컴포넌트 |
| 22일 | 유닉스 관리 권한 및 접근 권한, OSS : Open Source License, Using Open Source SW |
| 23일 | 프로그래머스 PS |
| 24일 | 첫 Pull Request |
| 25일 | 전북 해커톤 참여 |
| 26일 | 전북 해커톤 참여 |
| 27일 | 전북 해커톤 참여 |
| 28일 | 컴퓨터네트워크 : Unicast and Multicast, Transport Layer |
| 29일 | 소프트웨어공학 프로젝트 관리 및 계획, 요구사항 분석과 모델링 |
| 30일 | 유닉스 : Process and Redirection |
...
계획이란 문제를 해결하거나 목적을 이루기 위하여 앞으로 해야 할 일을 미리 생각하여 전략을 수립하고 그 내용을 결정하는 일이다.  

계획 수립은 목표를 달성하기 위해 필요한 자원의 할당, 일정 계획, 구성원들의 임무 등 프로젝트의 모든 활동을 구체화한 청사진이다.  

잘 수립된 계획은 실행을 용이하게 하고, 불필요한 작업을 감소시킴으로써 혼돈을 최소화하고 최적화한 프로젝트 환경을 제공한다.  

## 계획 활동

계획은 프로젝트의 핵심 활동이며 프로젝트 관리에서 가장 중요하게 여겨지는 과정이다. 또한, 목표를 세우고 이를 달성하기 위해 체계적인 진행 상황 관리를 할 수 있도록 세부 행동 방안을 마련하는 것이며 프로젝트 실행과 통제의 지침이 된다.  

프로젝트의 효율적인 관리는 프로젝트 계획을 철저히 세우는 것에서 시작하며, 성공하는 프로젝트는 훌륭한 프로젝트 계획에서 출발한다.  

## 계획과 제약 조건

현실적으로 프로젝트를 진행가기 전 신중하게 계획을 세우는 일을 소홀히 하기 쉽다. 그 결과 고객의 기대를 만족시키지 못하는 경우가 발생하고 프로젝트 진척이 어려워 납기가 지연되며 불행히도 많은 사람들도 좌절시키게 된다.  

프로젝트 투자할 수 있는 자원은 한정되어 있어 프로젝트 제약조건으로 인식된다. 모든 프로젝트는 시간, 돈, 얻어지는 품질의 3대 제약 조건 내에서 작업이 수행된다. 요즘에는 품질이 기본 조건으로 생각하고, 품질 대신 범위를 제약조건으로 넣어 생각하는 경우가 있다.  

## 계획 문서

프로젝트 계획은 프로젝트 시작 지점에 프로젝트 계획 문서를 만드는 과정이다. 계획 문서는 앞으로 전개되는 진행 상황에 대처하는 안내서이며 프로젝트를 모니터링하는 기준 문서가 된다.  

모니터링 활동은 프로젝트 시작부터 종료하는 시점까지 지속적으로 이루어져야 한다. 만약 프로젝트에 관한 정의와 계획이 없다면 프로젝트가 제대로 진행되는지 판단할 기준이 없다는 말과 같다.  

## 계획 이전 단계

발주자는 자신이 원하는 제품의 사양, 예산, 일정 등 프로젝트에 관한 요구사항을 정리한 문서인 제안요청서를 작성한다. 그 이후 후보 개발 업체들에게 제안요청서를 제시하는데, 이것을 입찰 공고라고 한다.  

후보 개발 업체들은 제안요청서를 기초로 소프트웨어 개발 계획을 담은 제안서를 만들어서 제출한다. 발주자는 소프트웨어 개발 회사를 선정하고 발주자와 개발 회사 간에 계약을 진행한다. 국제간의 대형 프로젝트의 경우 IEEE 국제 표준 규격에 따라 일을 진행하는 것이 일반적이다.  

## 계획 단계

프로젝트 헌장이 공식 승인되면 프로젝트 관리자는 이를 바탕으로 프로젝트 범위 개발, 프로젝트 관리 계획 등 프로젝트에서 요구하는 다양한 세부 계획을 수립하게 된다. 프로젝트 범위와 비용을 식별하고 일정을 개발하여 프로젝트 관리 계획을 수립한다.  

### 계획 단계의 관리 영역

프로젝트 계획 단계에서 고객의 요구에 대한 정확한 이해가 필요하며, 계획 단계에서 프로젝트 목표를 정화깋 파악하지 못하거나 잘못 파악하면 프로젝트를 실패로 이끌 가능성이 높아진다.  

소프트웨어 프로젝트의 경우 초기에 발견되는 요구는 일반적으로 피상적이거나 애매모호하며 구체적이지 못한 경우가 많다. 요구를 구체화 시킬수록 처음 생각한 것보다 많이 달라짐을 발견할 수 있으며 경우에 따라 요구가 변해 계획이 변경이 가해지기도 한다.  

프로젝트 계획 단계부터 요구에 대한 명확한 규명을 위해 구체화 작업, 조사와 연구가 필요하다.  

계획 수립 단계에서 프로젝트의 목적과 목표를 확립하고 핵심 문제를 파악해야 한다. 프로젝트의 영향을 미치는 제약 조건들을 평가하고 프로젝트의 위험 요소들을 예측 조사하고 대안을 마련한다.

프로젝트의 활동이나 산출물에 영향을 받을 수 있는 이해관계자들을 파악하고 분류하여 각 이해관계자가 전체 프로젝트 라이프 사이클 과정에서 효율적으로 참여할 수 있도록 관리 전략을 마련한다.  

구체적으로 어떤 계획서를 작성하느냐의 문제는 프로젝트의 규모, 예산 등에 따라 다르며 계획 수립 단계에서 결정되어야 한다.  

### 소프트웨어 개발 라이프 사이클

소프트웨어 개발 라이프 사이클의 계획 단계는 프로젝트 목표를 설정하고 이해관계자를 파악하며, 초기 요구사항을 분석하고, 일정과 예산을 산정하고, 프로젝트 팀을 구성하는 단계이다. 계획 단계는 프로젝트의 시작 승인을 받아 프로젝트 실행과 통제를 원활히 하기 위한 전략을 세우고 이를 문서화 하는 과정이다.  

프로젝트가 공식적으로 시작되면 프로젝트의 목표를 식별하고 프로젝트 관리자에게 프로젝트를 착수할 권한을 부여한다. 이러한 정보는 프로젝트 헌장에 기록되고 승인된다. 그 이후 프로젝트 관리자를 중심으로 프로젝트 관리 계획을 수립하고 프로젝트의 범위와 비용, 요구되는 활동들의 일정을 개발한다.  

## 프로젝트 추정

프로젝트 계획은 구체적이고 실현 가능한 것이어야 하며, 모순이 없도록 작성되어야 한다. 프로젝트 계획의 정확성은 프로젝트 추정의 정확성에 달려 있다.  

프로젝트 계획은 프로젝트 목표 달성을 위해 프로젝트 범위, 일정, 비용, 자원 요구 사항에 대한 구체성과 정확성을 제공해야 한다. 프로젝트 계획과 실제 실적을 비교하여 자료화하는 것을 성과 측정이라 한다.  

계획은 성과 기준을 제시하여 성과 측정의 기준이 되며 동시에 프로젝트 통제의 기준이 된다. 성과는 일의 범위, 일정, 예산 측면에서 측정된다.  

## 프로젝트 통합 관리 계획

프로젝트의 성공은 계획된 시간, 예산, 자원으로 원하는 품질의 제품을 얻어 고객을 만족시키는 일이다. 이를 위해 프로젝트 관리자는 프로젝트 범위, 일정, 예산, 품질, 인력, 의사 소통, 위기 관리 등을 통합하여 관리해야 한다. 이들 영역들은 모두 독립적인 요소들을 갖고 있어 각기 수립된 관리 계획과 절차들이 확립되어야 한다. 또한 모든 영역이 전부 서로 연계되어 있으며, 각 영역들을 통합적으로 관리해야 한다.  

계획 단계에서 프로젝트 통합 관리를 위해 작성되는 문서가 프로젝트 관리 계획서(PMP: Project Management Plan)이다.  

## 프로젝트 범위 관리 계획

범위 관리는 프로젝트의 성공을 위해 해야 할 일과 하지 말아야 할 일의 경계를 정하는 일이다. 프로젝트는 한정된 자원으로 수행되므로 범위 관리에서는 불필요한 일이 수행되지 않도록 프로젝트 범위를 계획 및 정의해야한다.  

확립된 범위를 검증하고, 승인된 범위의 변경을 적절히 통제할 수 있도록 계획한다. 프로젝트 결과물의 특징과 기능인 요구사항을 규명하는 일은 프로젝트 범위 관리의 기본이다.  

### 제품 범위와 프로젝트 범위

- 제품 범위 : 제품과 서비스 또는 프로젝트 결과물이 가지는 특징 및 기능

- 프로젝트 범위 : 지정된 특징 및 기능을 가진 결과물을 만들기 위해 수행해야 할 작업들까지 포괄하는 개념으로 정의

예를 들어 현금자동인출기를 만드는 프로젝트의 경우 `현금인출`, `계좌이체`가 제품범위, 프로젝트 범위는 프로젝트 결과물의 인도를 위해 필요한 전체 작업, 프로젝트 관리 계획, 프로젝트 라이프 사이클의 단계들 포함한다.  

소프트웨어 프로젝트의 경우 제품범위는 소프트웨어의 기능을 정의하기 위해 유스케이스(use case) 기법을 활용할 수 있다.  

### 작업 분류 체계 (WBS : Work Breakdown Structure)

소프트웨어 프로젝트의 경우 프로젝트 범위는 소프트웨어의 특징 및 기능을 만족시키기 위해 수행되어야 하는 작업들의 모임이다. 프로젝트 범위 계획은 요구사항을 수집하고 범위를 정의한 후 작업 분류 체계를 작성하는 과정이다.  

WBS는 프로젝트 팀에서 프로젝트의 목표를 달성하기 위해 수행해야 할 작업을 인도물 중심으로 계층적으로 작성한 구조이다. 또, WBS는 일정과 원가를 산정할 수 있는 수준까지 분할한다.  

WBS에 포함되어 있지 않은 것은 프로젝트 업무 범위 밖에 있다. 작업을 분류하고 식별할 때 해당 프로젝트의 참여자들이 전부 참여하여 의견을 공유하고, 책임과 역할에 대해 이해하여 작업을 수행할 수 있도록 하는 것이 바람직하다.  

## 프로젝트 일정 관리 계획

모든 프로젝트는 시작과 끝이 있다. 따라서 시작일과 종료일이 존재한다. 납기일을 무시하고 프로젝트를 진행할 수 있는 경우는 거의 없다. 프로젝트에 소요되는 기간과 비용은 대부분 프로젝트의 성패를 판단하는 중요한 기준이며, 프로젝트 관리의 핵심 사항이다.

## 프로젝트 일정 관리 순서

1. 활동 정의 : 프로젝트에 요구되는 활동들이 정의되어 작업패키지로 식별
2. 활동의 순서 배열 : 활동 간의 논리적인 관계와 의존도를 문서화하고 활동의 순서를 배열
3. 활동의 자원 산정 : 각 일정 활동을 수행하기 위해 필요한 자원의 수요를 산정
4. 활동의 기간 산정 : 각 활동에 배치된 자원을 고려하여 개별 활동 수행에 필요한 시간을 추정
5. 일정 개발 : 활동 순서, 자원 요구사항, 수행 기간 등 앞의 4단계 프로세스를 바탕으로 일정을 개발

프로젝트 일정 개발 순서도  
`활동 정의 -> 활동의 산정 -> 활동의 기간 산정 -> 일정개발` 이거나 `활동 정의 -> 활동의 순서 배열 -> 일정 개발`로 이뤄진다.  

## 프로젝트 품질 관리 계획

품질 관리 및 품질 보증은 제품을 만들어내는 조직에서는 필수적인 활동이다. 소프트웨어 공학 역시 고품질의 소프트웨어를 생산하는 것을 그 목표로 하고 있다. 품질은 요구사항에 대한 적합도 또는 사용성에 대한 적합도이다. 소프트웨어의 품질도 다른 제품들과 마찬가지로 사용자의 입장에서 보여지고 평가되어야 한다. 소프트웨어의 품질은 개발팀의 목적, 그리고 사용자의 기대 및 요구와 깊은 관계를 가지고 있다. 소프트웨어의 품질 관리는 소프트웨어를 만드는 공정, 소프트웨어를 만드는 조직의 특성을 중요시한다.  

## 현대적 품질 관리

현대적 품질 관리의 특징은 고객 만족, 경영층 참여, 검사 보다는 예방, 지속적인 개선 및 품질 계획에서 찾을 수 있다.  
1. 계획 단계에서 품질 관리 계획을 세워 이해관계자들의 품질 요구사항을 명확하게 정의하고 이를 실현시키기 위한 방법과 절차, 기준을 수립한다.
2. 프로젝트를 진행하며 품질 관리 계획을 바탕으로 품질 보증 활동을 수행한다.
3. 프로젝트의 결과물이 품질 표준에 적합한지 여부를 판단하여 적합하지 않을 경우 그 원인을 찾아내어 제거하는 품질 통제로 이루어져 있다.  

품질 보증은 품질 통제 활동을 통해 도출된 문제를 개선하여 얻어지는 결과이다.  

## 프로젝트 원가 관리

프로젝트에 소요되는 비용과 기간을 예측하기 위해서 프로젝트의 규모를 알아야 한다. 원가 산정은 프로젝트 라이프 사이클 각 단계에서 얼마만큼의 비용이 투입될 것인가를 추산하여 프로젝트 전체에 요구되는 비용을 예상하는 활동이다. 또한, 프로젝트의 계약이 이뤄지지 않은 단계에서도 타당성 검토를 위해 개략적으로 이뤄진다. 공식적인 승인이 이뤄진 후 계획 단계에서 구체적인 비용 측정을 위해 원가 산정이 이뤄진다.  
프로젝트가 수행되는 과정에서도 정확한 비용 수치가 계속 요구되어 원가 산정은 지속적으로 수행되는 활동이다.  

## 원가 추정 방법

원가 추정하는 가장 정확한 방법은 상향식 추정이다. 상향식 추정은 WBS의 최하위 수준으로 식별된 작업패키지 별로 원가를 산출하고 상위 단계로 합하여 전체 원가를 산출한다. 이를 위해 각 작업패키지에 대한 상세한 입력 자료가 필요하고 많은 비용과 시간이 소요된다.  

반면 프로젝트 착수 단계에서 정확히 원가를 산출할 수 있는 구체적인 근거가 없을 경우 전문가의 경험과 판단에 의지하는 하향식 추정 방법이 있다.  

소프트웨어 프로젝트의 경우 개발 역사가 짧고, 프로젝트가 전 산업 분야에 걸쳐 다양하고, 결과물이 눈에 보이지 않아 계획 단계에서 정확히 원가를 산출할 수 있는 자료가 없어 원가 산정에 하향식 추정 방법을 흔히 사용한다.  

### LOC(line of code)

소프트웨어의 크기와 규모를 설명하는 가장 간단한 하향식 추정 방법은 LOC이다. LOC는 소스 코드에 나타날 물리적 줄의 수이다. LOC 척도는 계획 단계에서 예측하기 어렵고, 경제적 의미가 미흡하고, 사용될 프로그래밍 언어에 의해 크게 영향을 받으며, 개발공수의 50% 이상이 투입되는 공정 과정(분석, 설계, 문서화, 관리, 품질 등)은 코드의 행 수로 측정할 수 없다.

### 기능 점수

소프트웨어의 규모를 측정하는 또 다른 하향식 추정 방법은 기능 점수 기법이다. 사용자 관점에서 요구되는 소프트웨어의 기능들을 측정하여 소프트웨어 개발 규모를 예측하는 것이다. LOC에 비하여 매우 정확히 초기 측정치를 산정할 수 있다는 것이 가장 중요한 장점이다. 그러나 계획 단계에서 정확한 기능 점수를 측정하는데 한계가 있으며, 요구사항 정의 단계에서 정확한 기능 점수 측정이 가능하다.  

## 형상 관리 계획

형상이란 사물이 생긴 모양 또는 이미지를 의미한다. 소프트웨어의 모습은 각 개발 단계마다 만들어지는 산출물인 문서로 나타나게 된다. 결국 소프트웨어의 형상은 문서이며, 프로젝트를 수행하며 개발 과정에서 생산되는 산출물인 문서를 관리하는 것을 소프트웨어 형상 관리라 할 수 있다.  

변경은 개발과정에서도 발생하기도 하고 발생된 후 유지보수 하는 과정에서도 필요하다. 따라서 변경은 불가피하다. 변경을 통제하는 절차는 매우 중요하다.  

## 확인 및 검증 계획

소프트웨어 개발은 여러 단계를 거쳐 이뤄진다. 추적성은 시스템 개발 프로세스 내에서 서로 다른 단계의 요구 사항들 사이에 연관 관계를 제공하는 기술이다. 사용자의 요구사항을 올바르게 이해하는 과정을 확인이라 한다. 이해된 요구사항을 소프트웨어에 제대로 반영하여 올바르게 제품을 만들어 갈 수 있도록 보장하는 활동을 검증이라 한다. 즉, 확인이란 소프트웨어가 고객의 기대와 요구사항을 만족시킬 수 있도록 보장하는 활동이고, 검증이란 소프트웨어가 지정된 기능을 정확히 수행할 수 있도록 단계별 점검을 통하여 추적하는 활동이다.  

## 소프트웨어 확인 및 검증

소프트웨어 확인 및 검증은 개발 대상 소프트웨어가 사용자 요구사항을 만족할 수 있도록 개발 생명주기 전반에 걸쳐 수행되는 점검과 분석 프로세스이며, 중요한 프로젝트 관리 활동 중 하나이다.  

소프트웨어 개발 조직은 사용자의 요구사항은 프로젝트 초기에 도출해 내야 한다.  

개발 과정 동안에 소프트웨어 산출물이 사용자 요구사항을 만족하는지, 각 단계이 산출물이 이전 단계 산출물의 요구 조건을 제대로 반영하여 만들어 졌는지 추적하는 것은 소프트웨어의 품질과 밀접한 관련을 맺고 있다.  
