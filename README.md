...
- 프로세스의 절차적인 요구 조건에 따라 모듈을 설꼐할 경우 모듈이 단지 절차의 한 부분이라는 이유로 설계되었다면 응집도가 낮은 절차적 응집도만을 가진 모듈로 될 수 있다.
...
- 프로세스 절차에 의해서만 모듈화를 하게 되면 설꼐된 모듈이 여러 임무를 수행할 수 있거나 한 임무의 일부분만을 수행할 수도 있다.
